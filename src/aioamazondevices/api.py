...
class AmazonEchoApi:
    ...
    async def get_devices_data(
        ...
        devices_list: dict[str, dict[str, Any]] = {}
        for data in json_data["devices"]:
            if data.get("deviceType") not in DEVICE_TO_IGNORE:
                devices_list[data.get("serialNumber")] = data
        ...
        for endpoint in endpoints["endpoints"]:
            ...
            if serial_number in devices_list:
                final_devices_list[serial_number] = await self._build_device(
                    endpoint, devices_list[serial_number]
                )

        return final_devices_list

    async def _build_device(
        self, endpoint: dict[str, Any], device_data: dict[str, Any]
    ) -> AmazonDevice:
        """Build AmazonDevice Object."""
        do_not_disturb_status = False
        online_status = False
        device_sensors: dict[str, AmazonDeviceSensor] = {}

        if endpoint["settings"]["doNotDisturb"]:
            do_not_disturb_status = endpoint["settings"]["doNotDisturb"]["toggleValue"]

        if endpoint["features"]:
            for feature in endpoint["features"]:
                if feature["name"] == "connectivity":
                    online_status = (
                        feature["properties"][0]["reachabilityStatusValue"] == "OK"
                    )
        ...
        return AmazonDevice(
            ...
            device_locale="en-GB",  # Just use locale?
            online=online_status,
            serial_number=device_data["serialNumber"],
            software_version=endpoint["softwareVersion"]["value"]["text"],
            do_not_disturb=do_not_disturb_status,
